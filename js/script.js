document.addEventListener('DOMContentLoaded', function () {
  ...
  function initSwiper() {
    ...
    ScrollTrigger.matchMedia({
        ...
        gsap.to(list, {
         ...
         boxes.forEach(box => {
           box.addEventListener('click', function() {
             // 기존 선택된 box에서 big 제거 후, 클릭된 box에 big 추가
             if (selectedBox) {
      ...
      '(max-width: 1079px)': function () {
        ...
        // 윈도우 사이즈에 맞춰 스타트업 스와이퍼 초기화
        window.addEventListener('resize', initStartupSwiper);
        window.addEventListener('load', initStartupSwiper);
      }
    });
  }
  

  // 초기 로드 및 리사이즈 이벤트에서 initSwiper 실행
  initSwiper();
  ...
  document.querySelector('.event_swiper').addEventListener('mouseenter', function() {
    document.querySelector('.event_prev').style.opacity = '1';
    document.querySelector('.event_next').style.opacity = '1';
  });
  document.querySelector('.event_prev').addEventListener('mouseenter', function() {
    document.querySelector('.event_prev').style.opacity = '1';
    document.querySelector('.event_next').style.opacity = '1';
  });
  document.querySelector('.event_next').addEventListener('mouseenter', function() {
    document.querySelector('.event_prev').style.opacity = '1';
    document.querySelector('.event_next').style.opacity = '1';
  });
  
  document.querySelector('.event_swiper').addEventListener('mouseleave', function() {
    document.querySelector('.event_prev').style.opacity = '0';
    document.querySelector('.event_next').style.opacity = '0';
  });
});
